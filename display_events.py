...
def display_events(audio_path, trained_path, figure_dimensions, out_path, list_of_effects=LIST_OF_EFFECTS, moving_interval=10, threshold=None, display=True):
    ...
    if display:
        ...
        for j in range(num_effects):
            ...
            plt.ylabel(list_of_effects[j] + " level", rotation=90)
        plt.xlabel("Time (s)")
        fig.savefig(out_path, dpi=100)
        plt.show()

    return consolidated_H
